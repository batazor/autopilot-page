...
The bot can identify the active account from the chief avatar shown in the top-left of the main city. This is a fast fallback before the full `who_i_am` OCR probe: if `active_player` is empty, the rolling frame matcher compares the visible avatar crop with the saved reference for each configured player on that device.

This only works reliably when every account on the same device has a different in-game avatar. If two accounts share the same avatar, the matcher intentionally refuses ambiguous matches and the normal `who_i_am` flow takes over.
