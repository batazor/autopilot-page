...
| **Live video (WebCodecs)** | [scrcpy](https://github.com/Genymobile/scrcpy) over WebSocket | **~30 fps with ~50 ms latency** — H.264 stream decoded in the browser via WebCodecs. Auto-enabled when the device's input/screenshot backend is set to `scrcpy` and the browser supports WebCodecs (Chrome / Edge / Safari). |
...
In Docker installs, rolling screenshots are shared through the `wos_temporal` volume mounted by both
`bot` and `api`. If the screenshot panel is blank after an upgrade, fetch the current
`docker-compose.prod.yml` and recreate the `bot` + `api` services so that volume is mounted.
